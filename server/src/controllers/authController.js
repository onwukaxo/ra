...
export async function register(req, res) {
  ...
  // 🔐 OTP via Otp collection (single source of truth)
  ...
  try {
    ...
    // swallow send error – client just sees "code sent"
  }
...
export async function forgotPassword(req, res) {
  try {
    ...
    if (user) {
      ...
      // (You can plug resetLink into your email template here if needed)
    }
...
/**
 * sendOtp: phone + password flow that stores OTP on User.
 * (Left as-is so you don’t break other parts of the app.)
 */
...
async function sendEmailOtp(email, code) {
  ...
  await transporter.sendMail({
    ...
    to: email,
    subject: 'Rations verification code',
    html: `<div style="font-family:sans-serif;font-size:14px">
      Your Rations verification code: <b>${code}</b> expires in 5 mins
    </div>`,
  })
}
...
async function sendSmsOtp(phone, code) {
  ...
  const payload = {
    ...
    sms: `Your Rations verification code is ${code}. Valid 5 mins`,
    type: 'plain',
    channel: 'generic',
  }
...
export async function resendOtp(req, res) {
  ...
  const now = Date.now()
  const info = resendWindow.get(userId) || { count: 0, start: now }

  if (now - info.start > 60 * 60 * 1000) {
    info.count = 0
    info.start = now
  }
  ...
  let channel = user.phone ? 'sms' : 'email'
  try {
    if (user.phone) {
