...
export default function Register() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const idf = form.identifier.trim()
      const password = form.password
      const payload = { name, phone: idf, password }
  ...
  return (
    <div className="max-w-md mx-auto bg-white border border-slate-100 rounded-2xl shadow-sm p-6">
      ...
      <form onSubmit={otpPhase ? handleVerify : handleSubmit} className="space-y-3">
        ...
        {otpPhase && (
          <div>
            <label className="block text-sm mb-1">Enter verification code ({channel?.toUpperCase()})</label>
            <input
              value={code}
            ...
            <div className="mt-2 flex items-center gap-2">
              <button type="button" onClick={handleResend} disabled={cooldown>0} className="text-xs px-2 py-1 rounded-full border border-slate-300 disabled:opacity-60">Resend Code</button>
