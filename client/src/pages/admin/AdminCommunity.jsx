    ...
    api.get('/admin/community')
      .then(res => {
        ...
        setTagOptions(prev => {
          ...
          const merged = Array.from(new Set([...base, ...uniq]))
          try { localStorage.setItem('tagOptions', JSON.stringify(merged)) } catch {}
          return merged
        })
      })
  ...
  const handleCreate = async (e) => {
    ...
    if (form.mediaUrl) {
      try {
        ...
        setErrors({ mediaUrl: 'Enter a valid http(s) URL' })
        return
      }
    }
    ...
    setForm({ title: '', content: '', tag: '', imageUrl: '', mediaUrl: '', mediaTitle: '', externalLinkUrl: '', externalLinkTitle: '' })
    setEditingId(null)
    load()
  }
