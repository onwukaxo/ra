...
export default function MenuItemCard({ item }) {
  ...
  const handleAdd = () => {
    if (!cartEntry) {
      ...
      addToCart(item, 1, isWings ? sauce : undefined)
    } else {
      // Increment quantity, keep existing sauce if any
      updateQuantity(item._id, quantity + 1, isWings ? currentSauce : undefined)
    }
  }
  ...
  const handleMinus = () => {
    ...
    if (newQty <= 0) {
      ...
      removeFromCart(item._id, isWings ? currentSauce : undefined)
    } else {
      // Decrement quantity, keep same sauce
      updateQuantity(item._id, newQty, isWings ? currentSauce : undefined)
    }
  }
      ...
      <div className="p-4 flex flex-col gap-2 flex-1">
        ...
        {isWings && quantity > 0 && (
          <div className="mt-2 flex items-center gap-2 text-xs">
            ...
            <select
              value={currentSauce}
              onChange={(e) => updateSauce(item._id, e.target.value)}
