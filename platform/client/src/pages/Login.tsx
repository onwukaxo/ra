  ...
  const handleSubmit = async (e: any) => {
    ...
    try {
      ...
      if (from && from !== '/') {
        ...
        else if (role === 'staff') navigate('/dashboard', { replace: true })
    ...
    <div className="max-w-md mx-auto bg-white border border-slate-100 rounded-2xl shadow-sm p-6">
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <p className="text-xs text-slate-600 mt-2">
          New restaurant? <Link to="/signup/owner" className="text-primary-600 font-medium">Create your restaurant</Link>
        </p>
      </form>
    </div>
  )
