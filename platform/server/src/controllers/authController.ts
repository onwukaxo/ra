...
export async function register(req: Request, res: Response, _next?: NextFunction) {
  try {
    ...
    return res.status(201).json({ success: true, message: 'Verification code sent', data: { userId: user._id, channel } })
  } catch (err) {
...
export async function platformSignup(req: Request, res: Response) {
  try {
    const ownerName = String(req.body.ownerName || '').trim()
    const ownerEmail = String(req.body.ownerEmail || '').trim().toLowerCase()
    const password = String(req.body.password || '')
    const restaurantName = String(req.body.restaurantName || '').trim()
    const restaurantSlugRaw = String(req.body.restaurantSlug || '').trim().toLowerCase()
    const phone = String(req.body.phone || '').trim()
    ...
    const secondaryColor = String(req.body.secondaryColor || '').trim()
    ...
    const baseSlug = (restaurantSlugRaw || restaurantName)
      .toLowerCase()
      .replace(/[^a-z0-9\s-]/g, '')
      .trim()
      .replace(/\s+/g, '-')
    ...
    const tenant = await Tenant.create({
      ...
      contact: { email: ownerEmail, phone: phone || undefined, country: country || undefined, city: city || undefined },
      settings: { currency: currency || undefined, timezone: timezone || undefined },
    })
    ...
    tenant.createdBy = owner._id as any
    await tenant.save()

    const token = generateToken(owner._id, String(owner.role))
    const safeUser = {
      _id: owner._id,
      name: owner.name,
      email: owner.email,
    ...
    return res.status(201).json({ success: true, data: { user: safeUser, tenant: safeTenant, token } })
  } catch (err) {
