...
export async function listPosts(req: Request, res: Response, _next?: NextFunction) {
...
export async function getPost(req: Request, res: Response) {
  ...
  const post = tenantId ? await CommunityPost.findOne({ _id: req.params.id, tenantId }) : await CommunityPost.findById(req.params.id)
  if (!post) {
...
export async function updatePost(req: Request, res: Response) {
  const post = await CommunityPost.findOne({ _id: req.params.id, tenantId: req.user?.tenantId })
  if (!post) {
    return res.status(404).json({ success: false, message: 'Post not found' })
  }
